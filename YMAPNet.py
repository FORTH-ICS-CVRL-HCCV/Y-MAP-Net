...
def preprocess_image(frame, target_size=(128, 128), add_borders=False):
    ...
    if (add_borders):
        ...
        image, keypointXMultiplier, keypointYMultiplier, keypointXOffset, keypointYOffset = resize_image_with_borders(
            image, target_size)
        ...
        image, keypointXMultiplier, keypointYMultiplier, keypointXOffset, keypointYOffset = resize_image_no_borders(
            image, target_size)
        #print("Recovered image size ",image.shape," ")
...
def subpixel_refinement(channel_heatmap, max_loc, neighborhood_size):
    ...
    neighborhood = channel_heatmap[max(0, y - neighborhood_size):min(channel_heatmap.shape[0], y + neighborhood_size +
                                                                     1),
                                   max(0, x - neighborhood_size):min(channel_heatmap.shape[1], x + neighborhood_size +
                                                                     1)]
...
def draw_floor_normal_axes(image, floor_normal_result, scale=60, margin=14):
    ...
    cv2.rectangle(image, (max(0, x0p), max(0, y0p)), (min(W - 1, x1p), min(H - 1, y1p)), (20, 20, 20), -1)
...
#----------------------------------------------------------------------------------------
def compute_floor_plane_normal(normal_x, normal_y, normal_z, floor_mask, ceiling_mask=None, wall_mask=None,
                               floor_threshold=30, ceiling_threshold=30, wall_threshold=30, min_pixels=50):
    """
    ...
    if total_px < min_pixels:
        return dict(normal=None, confidence=0.0, floor_pixels=floor_px, ceiling_pixels=ceil_px, wall_pixels=wall_px,
...
#----------------------------------------------------------------------------------------
def detect_fallen_person(person_mask, normal_x, normal_y, normal_z, floor_mask, furniture_mask=None,
                         person_threshold=30, floor_threshold=30, furniture_threshold=30, aspect_ratio_threshold=0.80,
                         orientation_angle_threshold=35.0, floor_overlap_threshold=0.20, normal_horiz_threshold=0.45,
                         furniture_overlap_threshold=0.25, min_bbox_area=400, dismiss_higher_angle_than=40,
    ...
    if person_pixel_count == 0:
        return dict(is_fallen=False, aspect_ratio=0.0, orientation_deg=0.0, floor_overlap=0.0, normal_horiz_frac=0.0,
                    furniture_overlap=0.0, centroid_y_frac=0.0, person_pixel_count=0, signals={})
    ...
    if bbox_area < min_bbox_area:
        return dict(is_fallen=False, aspect_ratio=round(bbox_w / max(bbox_h, 1.0), 3), orientation_deg=0.0,
                    floor_overlap=0.0, normal_horiz_frac=0.0, furniture_overlap=0.0, centroid_y_frac=0.0,
                    person_pixel_count=person_pixel_count, bbox_heatmap=(x0, y0, x1, y1), heatmap_size=(W,
                                                                                                        H), signals={})
...
def reseed_ranomizer():
    ...
    np.random.seed(((t & 0xff000000) >> 24) + ((t & 0x00ff0000) >> 8) + ((t & 0x0000ff00) << 8) +
                   ((t & 0x000000ff) << 24))
...
def draw_labeled_blobs(labeled_map, bounding_boxes):
    ...
    for i, (x, y, w_box, h_box) in enumerate(bounding_boxes):
        ...
        area = w_box * h_box
        cv2.putText(output_image, f'ID {i+1} / area {area}', (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1,
                    cv2.LINE_AA)
...
def drawSinglePlotValueList(valueListRAW, color, itemName, image, x, y, w, h, minimumValue=None, maximumValue=None):
    ...
    org = (int(x + len(valueList)),
           calculateRelativeValue(y, h, valueList[len(valueList) - 1], minimumValue, maximumValue))
    ...
    org = (1 + int(x + len(valueList)),
           1 + calculateRelativeValue(y, h, valueList[len(valueList) - 1], minimumValue, maximumValue))
    image = cv2.putText(image, message, org, font, fontScale, color, thickness, cv2.LINE_AA)
...
def arrange_windows(window_specs, screen_w=3840, screen_h=2400, offset_x=0, offset_y=10, margin=6, titlebar_h=30,
...
#----------------------------------------------------------------------------------------
def visualize_heatmaps(cfg, instanceLabels, imageIn, frameNumber, heatmapsOut, keypoint_names, threshold=0.0,
                       drawJoints=False, drawPAFs=False, resizeHighHeatmaps=True, showClassHeatmaps=True,
                       showDepthInsteadOfMask=False, screen_w=3840, screen_h=2400):
    i = 0
    ...
    for i in range(0, len(heatmapsOut)):
        ...
        if (drawPAFs and i >= 17 and i <= 28) or (drawJoints and i <= 17) or ((i >= 29) and ((showClassHeatmaps) or
                                                                                             (i <= 33))):  # 17 / 20
    ...
    if frameNumber == 1 and hm_specs:
        ...
        layout = arrange_windows(hm_specs, screen_w=screen_w - hm_x, screen_h=screen_h, offset_x=hm_x)
    ...
    for i, keypoint_name in enumerate(keypoint_names):
        if keypoint_name != "bkg":  #Ignore Background
            ...
            for peak_point in peak_points:
                ...
                cv2.circle(visualized_image, (int(x * frameWidth), int(y * frameHeight)), radius=5, color=(0, 255, 0),
                           thickness=-1)  # Draw a filled circle

                # Add text with keypoint name
                text_position = (int(10 + x * frameWidth), int(10 + y * frameHeight)
                                 )  # Position text slightly away from the circle
                depth = 255
                if keypoint_depths:
                    depth = int(keypoint_depths[i] + 125)
                cv2.putText(visualized_image, keypoint_name, text_position, cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                            (0, depth, 0), 1)
...
def visualize_monitors(frame, hm, monitors):
    ...
    for i, entry in enumerate(monitors):
        ...
        cv2.circle(frame, (int(x * frameWidth), int(y * frameHeight)), radius=5, color=(255, 255, 0),
                   thickness=-1)  # Draw a filled circle
        ...
        # Add text with keypoint name
        text_position = (int(int(x * frameWidth) - text_width[0] // 2), int(20 + y * frameHeight)
                         )  # Position text slightly away from the circle
...
class YMAPNet:
    ...
    def __init__(self, modelPath, threshold=30, keypoint_threshold=50.0, engine="tensorflow", profiling=False,
        ...
        self.keypoints_model = NNExecutor(engine=engine, profiling=profiling, inputWidth=self.cfg['inputWidth'],
                                          inputHeight=self.cfg['inputHeight'], targetWidth=self.cfg['outputWidth'],
                                          targetHeight=self.cfg['outputHeight'],
                                          outputChannels=self.cfg['outputChannels'], pruneTokens=pruneTokens,
                                          VRAMLimit=vram_limit, compileModel=compileModel)
        #---------------------------------------------------------------------
        """
    ...
    def process_tiled(self, frame, borders=False, overlap=3):
        ...
        processed_tiles = []
        for tile in tiles:
            self.input_image, keypointXMultiplier, keypointYMultiplier, keypointXOffset, keypointYOffset = preprocess_image(
                tile, target_size=self.input_size, add_borders=borders)
        ...
        self.imageIn, self.depthmap, self.heatmapsOut = convertIO(self.cfg, self.input_image, composite_heatmap,
        ...
        self.keypoint_results = castNormalizedCoordinatesToOriginalImage(
            frame, self.input_image, self.output_size, keypoint_results, keypointXMultiplier, keypointYMultiplier,
    ...
    def setup_threshold_control_window(self):
        ...
        cv2.createTrackbar("Person BBox Threshold", "Threshold Controls", int(self.bounding_boxes_threshold), 256,
                           nothing)
        cv2.createTrackbar("Person BBox Min Area", "Threshold Controls", int(self.bounding_boxes_minarea), 10000,
                           nothing)
    ...
    def process(self, frame, borders=False, static_frame_threshold=0.0, eco_max_skip=10):
        self.input_image, keypointXMultiplier, keypointYMultiplier, keypointXOffset, keypointYOffset = preprocess_image(
            frame, target_size=self.input_size, add_borders=borders)
        ...
        if static_frame_threshold > 0.0:
            if hasattr(self, '_prev_input_image') and self._prev_input_image is not None:
                frame_diff = np.mean(
                    np.abs(self.input_image.astype(np.float32) - self._prev_input_image.astype(np.float32)))
        ...
        self.imageIn, self.depthmap, self.heatmapsOut = convertIO(self.cfg, self.input_image,
                                                                  self.keypoints_predictions, self.heatmap_threshold)

        if "heatmaps" in self.cfg:
            self.denoised = self.process_denoising(self.imageIn, self.heatmapsOut[self.chanDenoiseR],
                                                   self.heatmapsOut[self.chanDenoiseG],
                                                   self.heatmapsOut[self.chanDenoiseB])
        """
        ...
        if (self.cfg['heatmapAddDepthmap']):
            self.keypoint_depths = retrieve_keypoint_depth(
        ...
        if self.estimate_person_id:
            self.labeled_map, self.bounding_boxes = segment_and_label_persons(
                self.person_union, threshold=self.bounding_boxes_threshold, min_area=self.bounding_boxes_minarea)
        """
    ...
    def encodeSkeletonAsDict(self, i, keypoint_names=None, denormalize=True, flipX=False):
        ...
        for kp_index, name in enumerate(keypoint_names):
            ...
            if denormalize:
                if None in (self.input_image.shape[1], self.input_image.shape[0], self.keypointXMultiplier,
                            self.keypointYMultiplier, self.keypointXOffset, self.keypointYOffset):
                ...
                x, y = castNormalizedXYCoordinatesToOriginalImage(self.input_image.shape[1], self.input_image.shape[0],
                                                                  self.keypointXMultiplier, self.keypointYMultiplier,
                                                                  self.keypointXOffset, self.keypointYOffset, x, y)
    ...
    def visualize(self, frame, show=True, save=False):
        ...
        if show:
            visualize_heatmaps(self.cfg, self.instanceLabels, self.imageIn, self.frameNumber, self.heatmapsOut,
                               self.keypoint_names, threshold=self.heatmap_threshold, drawJoints=self.drawJoints,
                               drawPAFs=self.drawPAFs, screen_w=self.screen_w, screen_h=self.screen_h)
        ...
        if ('heatmapAddNormals' in self.cfg) and (self.cfg['heatmapAddNormals']):
            ...
            if self.depth_iterations > 0:
                improved_depth = improve_depthmap(depthmap, normalX, normalY, normalZ, learning_rate=0.02,
                                                  iterations=self.depth_iterations)
            else:
        ...
        if not self.skeletons is None:
            if show:
                drawSkeletons(self.skeletons, self.keypoint_names, self.cfg["keypoint_parents"],
        ...
        if (self.description):
            if show:
                ...
                cv2.putText(textWindow, "%s" % self.description, (8, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.3,
                            (123, 123, 123), 1)
                cv2.imshow('Description', textWindow)

        #Do main visualization
        cv2.putText(frame, "NN @ %0.2f Hz" % self.keypoints_model.hz, (8, 23), cv2.FONT_HERSHEY_SIMPLEX, 1.0,
                    (123, 123, 123), 1)
        cv2.putText(frame, "NN @ %0.2f Hz" % self.keypoints_model.hz, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 1.0,
                    (0, 0, 0), 1)
        visRGB = visualization(frame, self.frameNumber, self.keypoint_results, self.keypoint_depths,
                               self.keypoint_names)  #, self.blobs

        if self.chanFloor >= 0 and len(self.heatmapsOut) > max(self.chanFloor, self.chanNormalX, self.chanNormalY,
                                                               self.chanNormalZ):
            _n_hm = len(self.heatmapsOut)
            _ceiling_mask = self.heatmapsOut[self.chanCeiling] if (self.chanCeiling >= 0
                                                                   and self.chanCeiling < _n_hm) else None
        ...
        if (self.illustrate):
            ...
            imagesF = [
                visRGB, normal_output, depthmap, improved_depth, union_pafs, union_joints, union_segms,
                self.person_union, self.heatmapsOut[self.chanText], self.heatmapsOut[self.chanVehicle],
                self.heatmapsOut[self.chanAnimal], self.heatmapsOut[self.chanObject],
                self.heatmapsOut[self.chanFurniture], self.heatmapsOut[self.chanAppliance]
            ]
            labels = [
                'RGB+Pose', 'Normals', 'Depth', 'Depth Improved', 'PAF Union', 'Joint Heatmap Union', 'Class Union',
                'Person', 'Text', 'Vehicle', 'Animal', 'Object', 'Furniture', 'Appliance'
            ...
            illu = compose_visualization(images, labels, description=self.description, bg_size=(1920, 1080),
                                         img_size=images_resized, frameRate=self.frameRateList,
                                         relative_border_size=20 / self.cfg['inputHeight'])
            if show:
        ...
        if show:
            if (self.frameNumber == 1):
                ...
                if is_compact:
                    ...
                    overflow = [(n, w, h) for n, w, h in primary_specs + processed_specs if n not in placed]
                    if overflow:
