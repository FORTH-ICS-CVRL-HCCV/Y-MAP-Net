...
def sample_paf_along_line(PAF_heatmaps, paf_index, x1, y1, x2, y2, num_samples=20):
    """
...
def assign_best_candidate_for_person(joint_candidates, person_label_map, person_id, W, H):
    """
...
def findClosestJoint(
    ...
    # ------------------------------------------------------------------
    child_name = (keypoint_names[child_joint_index] if child_joint_index is not None and keypoint_names is not None
                  and child_joint_index < len(keypoint_names) else None)
    anchor_name = (keypoint_names[joint_index]
                   if keypoint_names is not None and joint_index < len(keypoint_names) else None)
    ...
    if child_name is not None and keypoint_names is not None:
        ...
        if (sym_child_name and sym_child_name in keypoint_names and sym_anchor_name
                and sym_anchor_name in keypoint_names):
    ...
    for candidate in keypoint_candidates:
        ...
        dist = ((candidate_x - joint_x)**2 + (candidate_y - joint_y)**2)**0.5
        ...
        if PAF_heatmaps is not None and paf_index is not None and paf_index >= 0:
            paf_score = sample_paf_along_line(PAF_heatmaps, paf_index, joint_x, joint_y, candidate_x, candidate_y)
        ...
        cost = (dist + depth_weight * depth_penalty + limb_length_weight * length_penalty - paf_weight * paf_score -
...
def growSkeletons(skeletons, skeleton_explanation, keypoint_results, keypoint_depths, keypoint_names, keypoint_parents,
                  keypoint_children, depth_map, PAF_heatmaps=None, paf_parents=None):
    ...
    return (newJointExplanations, skeletons, skeleton_explanation, keypoint_results, keypoint_depths, keypoint_names,
            keypoint_parents, keypoint_children)


def is_skeleton_anatomically_valid(skeleton, keypoint_names, keypoint_parents, vert_tolerance=0.10, max_limb_ratio=3.5,
                                   max_pair_dist=None):
    """
...
def resolveJointHierarchy(keypoint_results, keypoint_depths, keypoint_names, keypoint_parents, keypoint_children,
                          depth_map, PAF_heatmaps=None, paf_parents=None, person_label_map=None,
                          normalized_confidence=True, verbose=False):
    skeletons = []
    ...
    if person_label_map is not None:
        ...
        if len(person_ids) > 0:
            ...
            if verbose:
                print("Segmentation suggests", numberOfSkeletons, "persons (labels:", person_ids, ")")
    ...
    if not use_segmentation:
        ...
        root_joints = [
            jID for jID, name in enumerate(keypoint_names)
            if jID < len(keypoint_results) and keypoint_parents.get(name, '') == name
        ]

        # Count peaks per joint
        joint_peak_counts = [
            sum(1 for x, y, v in keypoint_results[jID]
                if ((x != 0) or (y != 0)) and v != 0) if jID < len(keypoint_results) else 0
            for jID in range(len(keypoint_names))
        ]
        ...
        if best_root >= 0:
            ...
            if verbose:
                print("Seeding from root joint:", keypoint_names[mostPopularJoint], "with", numberOfSkeletons, "peaks")
        else:
            ...
            if verbose:
                print("No root joint peaks — seeding from most popular joint:", keypoint_names[mostPopularJoint])
        ...
        if verbose:
            ...
            print("Most popular joint is ", keypoint_names[mostPopularJoint], " ", mostPopularJoint)
            print("Keypoints have ", len(keypoint_results), " elements")
            print("Most popular joint has ", len(keypoint_results[mostPopularJoint]), " elements")
        ...
        for person_idx, person_id in enumerate(person_ids):
            for jID in range(len(keypoint_names)):
                ...
                best = assign_best_candidate_for_person(candidates, person_label_map, person_id, W, H)
                if best is not None:
                    x, y, v = best
                    base = jID * 3
        ...
        if verbose:
            print("Initialized", numberOfSkeletons, "skeletons from segmentation.")
    ...
    while newJointExplanations != 0:
        (newJointExplanations, skeletons, skeleton_explanation, keypoint_results,
         keypoint_depths, keypoint_names, keypoint_parents, keypoint_children) = growSkeletons(
             skeletons, skeleton_explanation, keypoint_results, keypoint_depths, keypoint_names, keypoint_parents,
             keypoint_children, depth_map, PAF_heatmaps=PAF_heatmaps, paf_parents=paf_parents)
...
def find_peak_point_coordinates_from_heatmaps(joint_prediction_heatmaps, threshold, nms_dist=0.10):
    """
            ...
            for contour in contours:
                ...
                if M["m00"] > 0:
                    ...
                    if score <= threshold:
                        ...
                        score = channel_heatmap[cy, cx]
                    if score > threshold:
                        raw_peaks.append((cx / heatmaps.shape[1], cy / heatmaps.shape[0], score))
...
def grow_skeletons_with_pafs_and_depth(skeletons, PAF_heatmaps, depth_map, keypoint_results, paf_parents):
    ...
    for skeleton_index, skeleton in enumerate(skeletons):
        for joint_index in range(len(skeleton) // 3):
            ...
            if depth_map is not None and 0 <= updated_joint_x < depth_map.shape[
                    1] and 0 <= updated_joint_y < depth_map.shape[0]:
...
def dump_inputs(keypoint_heatmaps, PAF_heatmaps, depth_map, keypoint_names, keypoint_parents, keypoint_children,
                paf_parents, folder_path='input_dump'):
    import os
...
def load_and_run(folder_path='input_dump', verbose=True):
    ...
    skeletons = resolveJointHierarchyNew(keypoint_heatmaps, PAF_heatmaps, depth_map, keypoint_names, keypoint_parents,
                                         keypoint_children, paf_parents, verbose=verbose,
                                         dump=False)  #Don't redump dumped data
...
def resolveJointHierarchyNew(keypoint_heatmaps, PAF_heatmaps, depth_map, keypoint_names, keypoint_parents,
                             keypoint_children, paf_parents, sanity_check=True, person_label_map=None, verbose=False,
                             dump=False, threshold=0.5, debug=False):

    if dump:
        dump_inputs(keypoint_heatmaps, PAF_heatmaps, depth_map, keypoint_names, keypoint_parents, keypoint_children,
                    paf_parents, folder_path='input_dump')
    ...
    if DBG:
        ...
        kp_shape = keypoint_heatmaps.shape if hasattr(keypoint_heatmaps,
                                                      'shape') else f"list len={len(keypoint_heatmaps)}"
        paf_shape = (f"list len={len(PAF_heatmaps)}, each={PAF_heatmaps[0].shape if PAF_heatmaps else 'n/a'}" if
                     isinstance(PAF_heatmaps, list) else (PAF_heatmaps.shape if PAF_heatmaps is not None else "None"))
        ...
        for j in range(min(17, kp_arr.shape[2])):
            ...
            print(
                f"    joint[{j:2d}] {keypoint_names[j] if j < len(keypoint_names) else '?':15s}  min={ch.min():.1f}  max={ch.max():.1f}  mean={ch.mean():.1f}"
            )
    ...
    paf_start = 17
    paf_parents_local = [(p - paf_start) if (p is not None and p >= paf_start) else -1
                         for p in paf_parents] if paf_parents is not None else None
    ...
    if sanity_check and PAF_heatmaps is not None:
        paf_heatmaps_np = (np.stack(PAF_heatmaps, axis=2) if isinstance(PAF_heatmaps, list) else PAF_heatmaps)
        if DBG:
            print(
        ...
        filtered_heatmaps = filter_joint_heatmaps_with_segmentation_and_pafs(
            keypoint_heatmaps[:, :, :17], paf_heatmaps_np, person_label_map, paf_parents_local, paf_children_local,
            paf_threshold=0.20)
        if DBG:
            for j in range(17):
                ...
                print(
                    f"    filt joint[{j:2d}] {keypoint_names[j] if j < len(keypoint_names) else '?':15s}  raw_max={ch_raw.max():.1f}  filt_max={ch_filt.max():.1f}  surviving_px={n_surviving}"
                )
    ...
    if DBG:
        ...
        for j, peaks in enumerate(keypoint_results):
            ...
            print(
                f"    joint[{j:2d}] {name:15s}  peaks={len(peaks):2d}  scores=[{', '.join(scores[:6])}{'...' if len(scores)>6 else ''}]"
            )
    ...
    skeletons = resolveJointHierarchy(keypoint_results, keypoint_depths, keypoint_names, keypoint_parents,
                                      keypoint_children, depth_map, PAF_heatmaps=PAF_heatmaps,
                                      person_label_map=person_label_map, paf_parents=paf_parents_local, verbose=verbose)
    ...
    MIN_JOINTS = 3
    skeletons = [
        sk for sk in skeletons if sum(1 for j in range(len(keypoint_names)) if sk[j * 3 + 2] > 0) >= MIN_JOINTS
    ]
    ...
    skeletons = [sk for sk in skeletons if is_skeleton_anatomically_valid(sk, keypoint_names, keypoint_parents)]

    if DBG:
        print(f"  After merge+validity filter (min {MIN_JOINTS} joints): {len(skeletons)} skeleton(s)")
        for si, sk in enumerate(skeletons):
            filled = sum(1 for j in range(len(keypoint_names)) if sk[j * 3 + 2] > 0)
            joints_str = ', '.join(f"{keypoint_names[j]}=({sk[j*3]:.2f},{sk[j*3+1]:.2f})"
...
if __name__ == '__main__':
    ...
    keypoint_results = [
        [(0.4192708333333333, 0.3020833333333333, 159.04083), (0.6171875, 0.2838541666666667, 141.2194),
         (0.24479166666666666, 0.2708333333333333, 48.50865), (0.6953125, 0.2786458333333333, 178.62257),
         (0.5442708333333334, 0.2760416666666667, 197.31694), (0.11979166666666667, 0.234375, 125.39257),
         (0.2708333333333333, 0.23177083333333334, 187.11096)],
        [(0.4244791666666667, 0.2994791666666667, 133.68227), (0.2421875, 0.2682291666666667, 43.22541),
         (0.6223958333333334, 0.2760416666666667, 166.05347), (0.7005208333333334, 0.265625, 148.4746),
         (0.5546875, 0.265625, 188.59534), (0.12760416666666666, 0.2265625, 142.28362),
         (0.28125, 0.22395833333333334, 179.24513)],
        [(0.4140625, 0.2994791666666667, 177.57248), (0.6145833333333334, 0.2786458333333333, 126.27818),
         (0.6848958333333334, 0.2682291666666667, 83.32413), (0.5364583333333334, 0.2682291666666667, 200.26768),
         (0.1171875, 0.2265625, 115.697136), (0.265625, 0.22395833333333334, 205.32957)],
        [(0.4296875, 0.3020833333333333, 47.09539), (0.8046875, 0.2838541666666667, 52.067062),
         (0.6328125, 0.2786458333333333, 136.86526), (0.7317708333333334, 0.2708333333333333, 203.60585),
         (0.5755208333333334, 0.2734375, 209.4301), (0.1484375, 0.2265625, 176.34573),
         (0.3020833333333333, 0.22395833333333334, 204.98811)],
        [(0.0859375, 0.3359375, 46.993103), (0.40625, 0.2994791666666667, 186.09299),
         (0.8177083333333334, 0.2786458333333333, 43.10502), (0.6067708333333334, 0.2786458333333333, 54.066452),
         (0.5286458333333334, 0.2708333333333333, 67.97044)],
        [(0.6015625, 0.328125, 152.63991), (0.4270833333333333, 0.3203125, 122.23184),
         (0.7916666666666666, 0.3125, 120.61464), (0.1796875, 0.28125, 165.14168),
         (0.3307291666666667, 0.2760416666666667, 186.39389)],
        [(0.5130208333333334, 0.328125, 165.70483), (0.6901041666666666, 0.3255208333333333, 165.68414),
         (0.3958333333333333, 0.3203125, 140.68881), (0.8307291666666666, 0.3098958333333333, 118.44491),
         (0.24479166666666666, 0.2890625, 72.42027), (0.10416666666666667, 0.2734375, 158.12773)],
        [(0.6119791666666666, 0.390625, 145.31255), (0.8072916666666666, 0.3802083333333333, 116.45909),
         (0.8619791666666666, 0.3567708333333333, 56.746452), (0.3541666666666667, 0.3619791666666667, 149.8564),
         (0.4375, 0.3489583333333333, 122.73482), (0.19010416666666666, 0.3489583333333333, 134.72284)],
        [(0.6328125, 0.3984375, 52.127586), (0.671875, 0.390625, 157.29565), (0.5, 0.3854166666666667, 135.04921),
         (0.3802083333333333, 0.3541666666666667, 156.30101), (0.8515625, 0.3515625, 115.70071),
         (0.22395833333333334, 0.3489583333333333, 100.81232), (0.0859375, 0.3385416666666667, 196.46118)],
        [(0.5989583333333334, 0.453125, 139.02786), (0.8463541666666666, 0.4296875, 75.02516),
         (0.3463541666666667, 0.4192708333333333, 153.04124), (0.8515625, 0.3828125, 40.18486),
         (0.19270833333333334, 0.4088541666666667, 122.81491), (0.4375, 0.3645833333333333, 100.87214),
         (0.2708333333333333, 0.2838541666666667, 43.518326)],
        [(0.8489583333333334, 0.4401041666666667, 63.063007), (0.6536458333333334, 0.4557291666666667, 172.68842),
         (0.21354166666666666, 0.40625, 69.52523), (0.8515625, 0.3802083333333333, 62.373905),
         (0.078125, 0.390625, 140.70024), (0.3984375, 0.3697916666666667, 68.664635),
         (0.2682291666666667, 0.2916666666666667, 51.803497)],
        [(0.59375, 0.4609375, 57.856064), (0.7604166666666666, 0.453125, 104.55321),
         (0.1484375, 0.4088541666666667, 42.906784), (0.3046875, 0.4140625, 82.46474),
         (0.4270833333333333, 0.3697916666666667, 108.73532), (0.96875, 0.359375, 64.48195)],
        [(0.7083333333333334, 0.453125, 72.64494), (0.5208333333333334, 0.4583333333333333, 81.163345),
         (0.2526041666666667, 0.4140625, 105.54861), (0.10416666666666667, 0.4088541666666667, 57.807915),
         (0.4036458333333333, 0.3723958333333333, 79.26636), (0.9609375, 0.3619791666666667, 62.384922)],
        [(0.7161458333333334, 0.5625, 124.48455), (0.5885416666666666, 0.5651041666666666, 112.03074),
         (0.13020833333333334, 0.5234375, 99.96415), (0.2994791666666667, 0.5208333333333334, 143.70576),
         (0.4270833333333333, 0.4140625, 133.75581), (0.9791666666666666, 0.3984375, 47.957527)],
        [(0.5963541666666666, 0.5703125, 50.468452), (0.7265625, 0.5625, 55.19754),
         (0.5104166666666666, 0.5625, 142.59222), (0.109375, 0.5260416666666666, 69.28831),
         (0.23958333333333334, 0.5104166666666666, 200.29053), (0.4114583333333333, 0.4114583333333333, 156.60355),
         (0.9661458333333334, 0.3958333333333333, 77.7554)],
        [(0.6848958333333334, 0.6770833333333334, 107.68108), (0.7760416666666666, 0.6692708333333334, 82.541016),
         (0.5963541666666666, 0.6510416666666666, 150.87589), (0.11979166666666667, 0.6171875, 84.609604),
         (0.2942708333333333, 0.6197916666666666, 122.23952), (0.3567708333333333, 0.4453125, 44.153313),
         (0.4166666666666667, 0.4635416666666667, 162.47462), (0.9713541666666666, 0.421875, 70.81378)],
        [(0.6822916666666666, 0.6796875, 128.42076), (0.7760416666666666, 0.6588541666666666, 115.65214),
         (0.5963541666666666, 0.6484375, 101.36485), (0.4921875, 0.6614583333333334, 193.99911),
         (0.11979166666666667, 0.6171875, 117.777725), (0.2734375, 0.6119791666666666, 188.1004),
         (0.4036458333333333, 0.4635416666666667, 195.92084), (0.9661458333333334, 0.421875, 100.40271)], [], []
    ]
    keypoint_names = [
