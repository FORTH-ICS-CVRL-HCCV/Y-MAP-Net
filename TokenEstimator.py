...
class TokenEstimator2D:
    ...
    def process(self, frame, token_threshold=0.45, visualization=True):
        ...
        self.input_image, keypointXMultiplier, keypointYMultiplier, keypointXOffset, keypointYOffset = preprocess_image(
            frame, target_size=self.input_size)
        ...
        self.description = decodeOneHotDescriptionToString(self.vocabulary, self.onehot,
...
def main_token_estimation():
    ...
    if (save):
        enable_screensaver()
        os.system(
            "ffmpeg -framerate 25 -i colorFrame_0_%%05d.png -vf scale=-1:720 -y -r 25 -pix_fmt yuv420p -threads 8 %s_lastRun3DHiRes.mp4"
            % videoFilePath)
