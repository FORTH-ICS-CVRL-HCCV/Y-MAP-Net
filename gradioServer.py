...
#----------------------------------------------------------------------
menuT[
    "RGB To Pose, Depth, Normals, Class and Token Estimation"] = "Μετατροπή έγχρωμων εικόνων σε Πόζα, Βάθος, Κάθετα Διανύσματα, Κλάσεις και Περιγραφή "
...
def describe_image(image, crop_button, threshold, keypoint_threshold, history):
    ...
    input_image = visualization(input_image, estimator_pose.frameNumber, estimator_pose.keypoint_in_nn_coordinate_image,
                                estimator_pose.keypoint_depths, estimator_pose.keypoint_names)

    visBGR = visualization(frame, estimator_pose.frameNumber, estimator_pose.keypoint_results,
                           estimator_pose.keypoint_depths, estimator_pose.keypoint_names)
    ...
    if depthmap is not None and normalX is not None:
        improved_depth = improve_depthmap(depthmap, normalX, normalY, normalZ, learning_rate=0.01, iterations=10)
    ...
    """ % t("RGB To Pose, Depth, Normals, Class and Token Estimation"))
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                threshold_sl = gr.Slider(label=t("Heatmap Joint Threshold"), minimum=0.0, maximum=255.0, value=20.0,
                                         step=0.1, interactive=True)
                keypoint_threshold_sl = gr.Slider(label=t("Keypoint Threshold"), minimum=0.0, maximum=255.0, value=30.0,
                                                  step=0.1, interactive=True)
