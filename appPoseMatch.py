...
class PoseMatcher:
    ...
    def tick(self):
        ...
        return PoseMatchResult(score, joint_scores, self._ref_angles, live_angles, angle_names)
...
def _draw_skeleton_colored(image, skeleton, keypoint_names, keypoint_parents, joint_scores=None, alpha=1.0):
    """
...
def _draw_reference_skeleton(image, skeleton, keypoint_names, keypoint_parents, tint=(180, 180, 180)):
...
def draw_pose_match_overlay(frame, result, estimator, matcher):
    ...
    sk = _best_skeleton(skels) if skels else None
    if sk is not None:
        _draw_skeleton_colored(frame, sk, estimator.keypoint_names, estimator.cfg["keypoint_parents"],
                               joint_scores=result.joint_scores)

    # ── 2. Reference ghost ────────────────────────────────────────────────────
    _draw_reference_skeleton(frame, matcher.ref_skeleton, estimator.keypoint_names, estimator.cfg["keypoint_parents"])
    ...
    for i, (label, score) in enumerate(panel_lines):
        ...
        if score is None:
            # Header
            cv2.putText(frame, label, (pad, y), font, font_scale, (0, 0, 0), thickness + 2, cv2.LINE_AA)
            cv2.putText(frame, label, (pad, y), font, font_scale, (255, 255, 255), thickness, cv2.LINE_AA)
        else:
            color = _score_to_bgr(score)
            # Mini bar
            bar_x0 = pad
            ...
            bar_y = y - line_h // 2
            cv2.rectangle(frame, (bar_x0, bar_y), (panel_w - pad, y - 2), (50, 50, 50), -1)
            ...
            cv2.putText(frame, text, (pad, y), font, font_scale * 0.85, (0, 0, 0), thickness + 1, cv2.LINE_AA)
            cv2.putText(frame, text, (pad, y), font, font_scale * 0.85, color, thickness - 1 if thickness > 1 else 1,
                        cv2.LINE_AA)
    ...
    cv2.rectangle(frame, (pad, score_y0), (panel_w - pad, score_y1), (50, 50, 50), -1)
    cv2.rectangle(frame, (pad, score_y0), (score_x1, score_y1), overall_color, -1)
    score_pct = f"SCORE  {result.score * 100:.0f}%"
    cv2.putText(frame, score_pct, (pad, score_y1 - 2), font, font_scale, (0, 0, 0), thickness + 2, cv2.LINE_AA)
    cv2.putText(frame, score_pct, (pad, score_y1 - 2), font, font_scale, (255, 255, 255), thickness, cv2.LINE_AA)
    ...
    if matcher.success_flash:
        ...
        cx, cy = (W - tw) // 2, (H + th) // 2
        cv2.putText(frame, msg, (cx, cy), font, font_scale * 3, (0, 0, 0), 8, cv2.LINE_AA)
        cv2.putText(frame, msg, (cx, cy), font, font_scale * 3, (255, 255, 255), 4, cv2.LINE_AA)

    # ── 6. Hint line ──────────────────────────────────────────────────────────
    hint = "R=capture ref   Q=quit"
    cv2.putText(frame, hint, (pad, H - pad), font, font_scale * 0.85, (0, 0, 0), thickness + 1, cv2.LINE_AA)
    cv2.putText(frame, hint, (pad, H - pad), font, font_scale * 0.85, (200, 200, 200), thickness - 1
                if thickness > 1 else 1, cv2.LINE_AA)
