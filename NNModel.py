...
def add_token_output(bridge_output, input_layer, numTokens, activation='leaky_relu', dropoutRate=0.2,
    ...
    for i in range(numTokens):
        ...
        x_token = layers.Dense(layer_width, name="Y_l1_t%u" % i)(x)
        ...
        if prev_output is not None:
            ...
            prev_transformed = layers.Dense(layer_width, name="Y_l2X_t%u" % i)(prev_output)
            ...
            x_token_residual = layers.Dense(layer_width, name="Y_l2F_t%u" % i)(x_token)
        ...
        for d in range(3, depth + 1):  # Loop from depth 3 to the minimum of (depth, 7)
            x_token_residual = layers.Dense(layer_width, name=f"Y_l{d}_t{i}")(x_token_residual)
            x_token_residual = apply_activation(x_token_residual, activation)
            x_token_residual = layers.LayerNormalization()(x_token_residual)
...
def build_resnethybrid_cnn(
    ...
    token_output = append_final_multihot_layer(
        glove_outputs, bridge_output=None, D=300, TokensOut=number_of_glove_output_tokens, Classes=numClasses,
...
def build_unet(inputHeight, inputWidth, inputChannels, outputWidth, outputHeight, numKeypoints, num16BitHeatmaps=1,
               ...
               midSectionRepetitions=3, gloveLayers=7, multihotLayers=2, multihotLayerWidth=0,
