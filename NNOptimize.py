...
def pruneModel(model, cfg, trainingDataset):
    ...
    pruning_params = {
        'pruning_schedule':
        tfmot.sparsity.keras.PolynomialDecay(initial_sparsity=initial_sparsity, final_sparsity=final_sparsity,
    ...
    model.fit(trainingDataset, epochs=cfg['epochs'], batch_size=cfg['batchSize'], callbacks=pruning_callback, verbose=1)
    return model
...
def clusterModel(model, cfg, trainingDataset):
    ...
    rmsprop = tf.keras.optimizers.RMSprop(learning_rate=cfg['learningRate'], rho=0.9,
    ...
    clustered_model.compile(optimizer=rmsprop, loss='mse', metrics=['mae', 'acc'])
    clustered_model.fit(trainingDataset, epochs=cfg['epochs'], batch_size=cfg['batchSize'], verbose=1)
...
def quantizeModel(model, cfg, trainingDataset):
    ...
    q_aware_model.fit(trainingDataset, epochs=1, batch_size=cfg['batchSize'], verbose=1, validation_split=0.1)
...
def convertToTensorRT(model, trainIn=0, precision="fp32"):
    try:
        ...
        converter = trt.TrtGraphConverterV2(input_saved_model_dir="tensorRTIntermediateTFModel",
...
def loadRepresentativeData(cfg, num_samples=200):
    ...
    db = DataLoader(
        (cfg['inputHeight'], cfg['inputWidth'],
         cfg['inputChannels']), (cfg['outputHeight'], cfg['outputWidth'],
                                 cfg['outputChannels']), output16BitChannels=cfg['output16BitChannels'], streamData=1,
        batchSize=cfg['batchSize'], numberOfThreads=1, gradientSize=cfg['heatmapGradientSizeMinimum'],
        PAFSize=cfg['heatmapPAFSizeMinimum'], doAugmentations=0, addPAFs=int(cfg['heatmapAddPAFs']), addBackground=int(
            cfg['heatmapGenerateSkeletonBkg']), addDepthMap=int(cfg['heatmapAddDepthmap']), addDepthLevelsHeatmaps=int(
                cfg['heatmapAddDepthLevels']), addNormals=int(cfg['heatmapAddNormals']), addSegmentation=int(
                    cfg['heatmapAddSegmentation']), datasets=cfg["TrainingDataset"],
        libraryPath="datasets/DataLoader/libDataLoader.so")

    gen = TrainingDataGenerator(cfg=cfg, db=db, batch_size=cfg['batchSize'], numberOfTokens=cfg["tokensOut"],
                                numberOfClasses=cfg["tokensClasses"], workers=1, use_multiprocessing=False,
                                max_queue_size=1)
